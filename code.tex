...
ここで、独立同分布な確率変数列$\mathcal{D}\coloneqq\{X_i(\omega)\}_{i=1}^n$を考える。各$X_i$は$X_i:\Omega\rightarrow\mathcal{X}\times\mathcal{Y}$となる可測関数である。この確率変数列$\mathcal{D}(=\{X_i\}_{i=1}^n)$を入力データと呼ぶ。
...
汎関数$F:Y\rightarrow\mathbb{R}$の集合を$Y^*$と書く。位相は弱位相であるとし、$\mathcal{F}_1$-可測な確率変数$F^*:\Omega\rightarrow Y^*$によって定義される$F_\mathcal{D}\coloneqq F^*(\omega)$に対して
\begin{align}
\hat{f}\coloneqq\argmin_{f\in Y_M}F_\mathcal{D}(f)
\end{align}
