...
連続写像$f\colon\mathcal{X}\to\mathcal{Y}$の満たす集合全体を$Y$と表記する。モデルに対応した$Y$の部分集合(詳細は後述)を$Y_M$と書く。

汎関数$F\colon Y\to\mathbb{R}$の集合を$Y^*$と書く。位相は弱位相であるとし、$\mathcal{F}_1$-可測な確率変数$F^*\colon\Omega\to Y^*$によって定義される$F_\mathcal{D}\coloneqq F^*(\omega)$に対して
\begin{align}
